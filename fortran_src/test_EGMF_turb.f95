...
call initiate()
open (unit=117,file="output/lepton_deflection")
nmax=1000000
...
do i=1, nmax
   ...
   posini(2)=ran0()*i*Mpc
   posini(3)=ran0()*i*Mpc
   call selectEGMF(posini,b)
   write(unit=117,fmt=*) posini, b, sum(b**2)
   if (modulo(i*100, nmax) == 0) then
      call print_time(i)
   end if
end do
!$OMP end PARALLEL do
